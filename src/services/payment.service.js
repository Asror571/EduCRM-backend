  ...
  try {
    ...
    for (const groupId of student.currentGroups) {
      ...
      if (group && group.status === "active") {
        totalFee += group.pricing?.monthlyFee || 0;
      }
    }
...
const calculateLateFee = async (studentId, daysLate, _baseFee) => {
  try {
    ...
    // Check grace period
    if (daysLate <= organization.financialSettings?.gracePeriodDays || 0) {
      return {
        amount: 0,
        daysLate: 0,
      };
    ...
    const actualDaysLate =
      daysLate - (organization.financialSettings?.gracePeriodDays || 0);
    const lateFeePerDay = organization.financialSettings?.lateFeePerDay || 0;
