...
const sendPaymentReceiptSMS = async (student, payment) => {
  ...
  return await sendSMS(student.user.phone, message);
};
...
const sendClassReminderSMS = async (student, group, classDate) => {
  ...
  const message = `Eslatma: Bugun (${date}) ${group.schedule[0].startTime} da ${group.name} guruhida darsiz bor. Kechikmasdan keling!`;
  return await sendSMS(student.user.phone, message);
};
...
const sendDebtReminderSMS = async (student, debtAmount, daysOverdue) => {
  const message = `Hurmatli ${student.user.firstName}! Sizning ${debtAmount.toLocaleString()} UZS qarzdorligingiz ${daysOverdue} kun o'tdi. Iltimos, to'lovni amalga oshiring.`;
  return await sendSMS(student.user.phone, message);
};
...
const sendBirthdaySMS = async (student) => {
  const message = `Tug'ilgan kuningiz bilan tabriklaymiz, ${student.user.firstName}! Sizga baxt, omad va muvaffaqiyatlar tilaymiz! 🎉🎂`;
  return await sendSMS(student.user.phone, message);
};
